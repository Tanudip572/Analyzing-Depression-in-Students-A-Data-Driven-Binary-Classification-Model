...
data=data%>%select(-Age,-Academic.Pressure,,-Financial.Stress)
library(caTools)
set.seed(123)
...
model <- glm(Depression ~ CGPA+Study.Satisfaction+Work.Study.Hours ,
  data = train_data,
  family = binomial
)
options(scipen = 999)
...
vif_data <- data.frame(
  Variable = c("CGPA", 
               "Study.Satisfaction", "Work.Study.Hours"),GVIF = c( 1.001637, 1.006376 , 1.004781))
...
# 1. Get the model's training data (automatically excludes NAs)
model_data <- model.frame(model)  # Ensure rows align
outcome_name <- names(attr(terms(model), "dataClasses"))[1]  # Extract outcome name
